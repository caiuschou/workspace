...
//!
//! Design: [docs/rust-langgraph/13-react-agent-design.md] §8.2 stage 2.4.
...
async fn mock_tool_source_call_tool_any_name_returns_same_result() {
    ...
    let r2 = source.call_tool("other_tool", json!({"x":1})).await.unwrap();
...
async fn mock_tool_source_new_custom_tools_and_result() {
    ...
    let result = source.call_tool("search", json!({"q":"rust"})).await.unwrap();
