...
//! ```
//!
//! No API key is required: the example uses `SequenceMockLlmClient` from langgraph, which returns
//! predefined strings in order (first "Action: calculator(...)", then "Answer: N").

use std::sync::Arc;

use langgraph::{AsyncAgent, CalculatorTool, SequenceMockLlmClient, ToolRegistry};
