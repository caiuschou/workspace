...
基于 [agent-communication.md](../../docs/agent-communication.md) 与 [registration-discovery.md](../../docs/registration-discovery.md)，为 Agora 设计一组最简 Rust API：仅定义协议消息类型与序列化，供 `agora` crate 与 `agora-server` 共用。
...
不引入 tokio、tungstenite 等，保持 `agora` 为纯类型 + 序列化 crate。
...
| 下一步 | 在 `agora/src` 中实现上述类型与枚举，并补充单元测试（解析/序列化往返与示例 JSON）。 |

完成本方案后，在 `docs/` 的任务表中可将「Agora 最简 Rust API 方案」标为完成，并新增「实现 agora 消息类型与序列化」任务。
