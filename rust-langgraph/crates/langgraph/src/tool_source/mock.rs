...
//!
//! Design: [docs/rust-langgraph/13-react-agent-design.md] §8.2 stage 2.3.
...
impl MockToolSource {
    /// Build a mock that lists one tool `get_time` and returns fixed time string on call.
    ///
    /// Aligns with 13-react-agent-design §8.2: "固定工具列表" and "call_tool 返回固定文本".
    ...
    /// Build a mock with custom tool list and fixed call result.
