...
- **`ChatOpenAI`**：真实 OpenAI Chat Completions API，需启用 feature `openai`，默认从环境变量 `OPENAI_API_KEY` 读 key，也可用 `with_config` 自定义；可选 `with_tools(tools)` 开启工具调用。
- **`ChatZhipu`**：智谱 GLM Chat Completions API（OpenAI 兼容），需启用 feature `openai`，默认从环境变量 `ZHIPU_API_KEY` 读 key，base URL 指向 `https://open.bigmodel.cn/api/paas/v4`；支持 `glm-4-flash`、`glm-4-plus` 等模型。
...
- `mock.rs`：MockLlm。
- `openai.rs`：ChatOpenAI（仅在 feature `openai` 下编译）。
- `zhipu.rs`：ChatZhipu（仅在 feature `openai` 下编译）。
